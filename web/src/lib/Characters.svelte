...
{#if loading}
  <p class="text-sm text-muted">Henter karakterer…</p>
{:else if error}
  <p class="rounded-xl border border-bad/40 bg-bad/10 px-4 py-3 text-sm text-bad">{error}</p>
{:else if characters.length === 0}
  <div class="card p-8 text-center">
    <p class="text-muted">Ingen karakterer endnu. Hent fra Enka eller upload en .good-fil.</p>
  </div>
{:else}
  <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
    {#each characters as character (character.id)}
      <article class="card p-4">
        ...
        <p class="mt-1 text-sm text-muted">Niveau {character.level}</p>
